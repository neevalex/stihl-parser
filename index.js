		...
		await request( options )
			.then(
				async function (response) {
					...
					$( '.js-productContent .article' ).each(
						function (i, element) {
							id = $(this).attr('data-productid');
							title      = $( this ).find( '.productName a' ).text();
							url =  $(this).find('.productName a').attr('href');
							image = $(this).find('.imageContainer img').attr('src');
							
							if ($(this).find('.technicalInfo').html()) {
								smallDesc = $(this).find('.technicalInfo').html();
							} else { smallDesc = ''; }
							...
							price = $(this).find('.priceLabel span').html();
							price = price.replace('$', '');
...
app.get('/api', async function(req, res) {
	...
	if (req.query.cmd == 'export') {
		const csvFromArrayOfObjects = convertArrayToCSV(db.get('products').value());
			  ...
			  res.setHeader('Content-disposition', 'attachment; filename=stihl-export.csv');
